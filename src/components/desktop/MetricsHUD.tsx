...
import { AlertTriangle, Box, Layers, Maximize2, X } from 'lucide-react';
...
export const MetricsHUD: React.FC<MetricsHUDProps> = ({ project, metrics }) => {
  ...
  // Reset dismissed state when switching projects
  useEffect(() => {
  ...
  return (
    <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 max-w-sm pointer-events-none select-none">
      ...
      {metrics?.isHeavy && !isWarningDismissed && (
        <div className="bg-amber-950/90 backdrop-blur-md border border-amber-600/70 text-amber-200 text-xs rounded-xl p-3 shadow-lg flex items-start justify-between gap-2.5 pointer-events-auto transition-all animate-in fade-in duration-200">
          ...
          <button
            ...
            className="text-amber-400/80 hover:text-amber-100 hover:bg-amber-900/60 p-1 rounded-lg transition-colors shrink-0 cursor-pointer ml-1"
